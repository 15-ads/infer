...
def build_prompt(actions, duration, use_detailed=False):
    ...
    prompt = (
        ...
        f"**关于‘球’的说明：球通常是白色或黄色的小型圆形物体（类似网球大小），在地面、空中、狗嘴里或人手中。即使球快速运动、部分被遮挡、或距离较远导致看起来模糊，只要出现圆形小物体且颜色符合，都应当认定为球。**\n\n"
        ...
        f"**注意：如果看到疑似球的物体（即使不十分清晰），请按最可能的选项回答，不要轻易说‘没有看到球’。只有确实从头到尾没有任何圆形小物体，才回答‘完全没有看到球’。**\n\n"
        f"然后，用一两句话描述狗正在做什么（例如：奔跑、慢走、站立、嗅闻、跳跃、张嘴、空咬等）。\n\n"
        f"【步骤2】判定动作是否出现\n\n"
        f"你需要在以下动作中判断哪些确实出现：\n{actions_text}\n\n"
        f"每个动作的定义和判定规则：\n\n"
        f"## 追球\n"
        f"- **必要条件**：\n"
        f"  1. 球存在（步骤1结论不是‘完全没有看到球’）\n"
        f"  2. 球不在狗嘴里\n"
        f"  3. 狗正在快速奔跑或持续移动，且方向朝向球\n"
        f"  4. 狗表现出兴奋/专注状态\n"
        f"- **常见误判排除**：缓慢走向球、只是看着球、球已经在嘴里 → 不算追球\n\n"
        f"## 拉球\n"
        f"- **必要条件**：\n"
        f"  1. 球在狗嘴里（可以是部分可见）\n"
        f"  2. 狗身体向后倾斜、头部或颈部向后拉扯\n"
        f"  3. 有明显的对抗性发力动作（四肢抓地、身体后坐）\n"
        f"- **注意**：仅嘴里有球但无拉扯动作 → 不算拉球\n\n"
        f"## 犬咬\n"
        f"- **必要条件**：\n"
        f"  1. **球在狗嘴里**（必须看到圆形物体在口部区域，或狗嘴闭合时呈现球体轮廓）\n"
        f"  2. 狗含住或咬住球，但没有连续向后拉扯（如果有拉扯，则属于拉球）\n"
        f"- **绝对不算犬咬的情况**：\n"
        f"  - 步骤1结论是‘完全没有看到球’\n"
        f"  - 狗只是张嘴、空咬、舔舐，但嘴里没有任何物体\n"
        f"  - 狗嘴里有其他非球物体（如玩具、食物）\n\n"
        f"**全局硬性规则**：\n"
        f"1. 如果步骤1中球的位置是‘完全没有看到球’，则追球、拉球、犬咬全部判‘未出现’。\n"
        f"2. 如果球存在但不在嘴里，犬咬一定不出现。\n"
        f"3. 即使球存在，犬咬也必须基于**看到球在嘴里**的直接证据，不能因为狗做出咬合动作就推断。\n\n"
        f"**请按以下格式写出判定文字（先重申步骤1的球位置）：**\n"
        ...
        f"- 忽略画面水印时间戳\n"
        f"- 根据实际动作起止帧判断，例如追球从第2秒开始奔跑，到第3.5秒停止或咬住球结束。\n\n"
        f"先写文字说明每个动作的起止时间：\n"
        f"- 追球：未出现 / X秒到Y秒\n"
        f"- 拉球：……\n"
        f"- 犬咬：……\n\n"
