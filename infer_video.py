...
def build_prompt(actions, duration, use_detailed=False):
    ...
    prompt = (
        ...
        f"  - 被训导员拿在手里\n"
        f"  - **完全没有看到球（球未出场）**\n\n"
        f"**重要警告**：如果球完全没有出现（即视频中从头到尾看不到任何球或类似物体），步骤2中**禁止**判定任何与球相关的动作（追球、拉球、犬咬），只能判定为\"无球\"或\"不属于上述动作\"。\n\n"
        f"然后，用一两句话描述狗正在做什么（例如：奔跑、慢走、站立、嗅闻、跳跃、咬住后拉扯等）。\n\n"
        f"【步骤2】判定动作是否出现（必须写文字 + 理由）\n\n"
        f"你需要从下面列表中判断哪些动作**确实出现**：\n"
        f"{actions_text}\n\n"
        f"每个动作的定义如下（参考论文的三种描述方式）：\n\n"
        f"## 追球\n"
        f"- **属性（关键视觉线索）**：球不在嘴里；狗处于奔跑或快速移动状态；身体姿态向前、兴奋（尾巴翘起、耳朵竖起等）。\n"
        f"- **描述（时间过程）**：狗从静止或慢走开始加速，朝着球的方向快速奔跑，期间球始终未被叼住，狗持续追逐但未咬到。\n"
        f"- **上下文（区分规则）**：与\"拉球\"的区别在于没有咬住后的拉扯；与\"犬咬\"的区别在于球未在嘴里。\n\n"
        f"## 拉球\n"
        f"- **属性**：球已经在狗嘴里；狗身体向后倾斜、四肢抓地、头部和颈部向后用力；有明显的拉扯动作。\n"
        f"- **描述**：狗咬住球后，通过后撤身体、扭动头部或甩动颈部来对抗阻力（可能是人拉或球被固定）。\n"
        f"- **上下文**：仅嘴里有球但不向后发力不算拉球；必须有\"咬住后拉扯\"的直接画面证据。\n\n"
        f"## 犬咬\n"
        f"- **属性**：球完全在狗嘴里，狗闭嘴含住或用牙齿咬住；可能伴随咀嚼或叼着走，但没有向后拉扯的发力动作。\n"
        f"- **描述**：狗用嘴含住或咬住球，保持球在口中，可以静止、行走或轻微摇头，但身体重心不向后对抗。\n"
        f"- **上下文**：犬咬是一种**状态**（球在嘴里），而拉球是一种**动态动作**（咬住后拉扯）。如果球已在嘴里，绝不可能同时是\"追球\"。\n\n"
        f"**判定铁律（必须逐条遵守）：**\n"
        f"1. **如果步骤1中明确写了\"完全没有看到球（球未出场）\"，则追球、拉球、犬咬全部强制判为\"未出现\"，不得有任何例外。**\n"
        f"2. **证据必须来自步骤1**：步骤1中没提到的视觉信息，不能作为判定依据。\n"
        f"3. **动作必须实际完成**：准备、试图、快要做了都不算。\n"
        f"4. **上述定义中的所有属性必须同时满足**，才判为\"出现\"。\n"
        f"5. 如果步骤1的描述与任何属性矛盾（例如说\"球在地上、狗未接触\"），该动作直接判\"未出现\"。\n"
        f"6. 如果狗的行为不属于上述任何动作，请明确写\"不属于上述动作\"，并描述实际行为。\n\n"
        f"**请按以下格式写出判定文字：**\n"
        f"- 追球：出现/未出现。理由：对照属性，……；对照描述，……；对照上下文，……\n"
        f"- 拉球：出现/未出现。理由：……\n"
        f"- 犬咬：出现/未出现。理由：……\n\n"
        f"【步骤3】给出动作的时间区间（仅当动作出现时）\n\n"
        f"**时间计算规则（严格遵守）：**\n"
        f"- **起始时间必须从本片段的第一帧（0秒）开始计算**，绝对禁止使用画面上的水印时间戳（如 15:03:38）作为参考。\n"
        f"- **如果你看到画面上有录像时间戳（例如右上角的数字），请完全忽略，不要写进回答里。**\n"
        f"- 根据视频中狗的实际动作变化来判断时间：例如，狗从第 2 秒开始加速奔跑（此时为追球开始），到第 3.5 秒咬住球停下（追球结束）。\n"
        f"- 结束时间 ≤ {duration:.0f} 秒。\n"
        f"- 如果你不确定具体秒数，可以给出估计值（保留一位小数），但必须基于动作的起止帧。\n\n"
        f"**先写文字说明每个动作的起止时间：**\n"
        f"- 追球：2 秒到 3.5 秒\n"
        f"- 犬咬：未出现\n"
        f"- 拉球：4 秒到 5 秒\n\n"
