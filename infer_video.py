...
def build_prompt(actions, duration, use_detailed=False):
    ...
    prompt = (
        ...
        f"**判定铁律：**\n"
        f"1. **球存在性检查**：必须先完成步骤1的球存在性判断。如果\"无球\"，直接输出所有球相关动作=未出现，跳过后面的属性比对。\n"
        f"2. **证据一致性**：步骤2的理由中引用的所有视觉证据（如“球在嘴里”、“奔跑方向”）必须在步骤1中明确出现过。\n"
        f"3. **动作必须实际完成**：准备、试图、快要做了都不算。\n"
        f"4. **所有属性必须同时满足**，才判为\"出现\"。\n"
        f"5. 如果狗的行为不属于上述任何动作，请明确写\"不属于上述动作\"，并描述实际行为。\n\n"
        f"**请按以下格式写出判定文字：**\n"
        f"- 球存在性检查：步骤1结果是 [有球/无球]。\n"
        f"- 追球：出现/未出现。理由：（如果无球，直接写\"因为无球，所以未出现\"；如果有球，逐条对照属性）\n"
        f"- 拉球：出现/未出现。理由：……\n"
        f"- 犬咬：出现/未出现。理由：……\n\n"
        ...
        f"- 例如：从第 0 秒到第 2 秒狗在慢走，第 2 秒开始加速奔跑（追球开始），第 3.5 秒咬住球（追球结束）。\n"
        f"- 结束时间 ≤ {duration:.0f} 秒。\n"
        f"- 如果动作跨越多个片段？不需要考虑，只判断当前片段。\n\n"
        f"**先写文字说明每个动作的起止时间：**\n"
        f"- 追球：未出现\n"
        f"- 拉球：未出现\n"
        f"- 犬咬：未出现\n\n"
