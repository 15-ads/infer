...
def build_prompt(actions, duration, use_detailed=False):
    ...
    prompt = (
        ...
        f"请严格按以下三个步骤分析，每步都必须用文字写出，最后输出 JSON。\n\n"
        f"[步骤1] 观察并描述球的位置和狗的动作\n\n"
        f"关于球的说明：球通常是白色或黄色的小型圆形物体（类似网球大小），可能在地面、空中、狗嘴里或人手中。即使球快速运动、部分被遮挡、或距离较远导致看起来模糊，只要出现圆形小物体且颜色符合，都应当认定为球。\n\n"
        f"请明确写出一句话回答球的位置（从以下选项中选择）：\n"
        f"- 在狗的嘴里（狗已经叼住）：必须看到圆形物体在狗口中或其轮廓\n"
        f"- 在地上（狗尚未接触）：球静止或滚动在地面\n"
        f"- 在空中/被抛出（狗正在追逐）：球在飞行轨迹中\n"
        f"- 被训导员拿在手里：球被人手拿着\n"
        f"- 完全没有看到球：视频中没有任何类似白色/黄色圆形物体的东西\n\n"
        f"注意：如果看到疑似球的物体（即使不十分清晰），请按最可能的选项回答，不要轻易说'没有看到球'。\n\n"
        f"然后，用一两句话描述狗正在做什么（例如：冲向球、奔跑、慢走、站立、嗅闻、跳跃、张嘴、空咬等）。\n\n"
        f"[步骤2] 判定动作是否出现\n\n"
        f"你需要从下面列表中判断哪些动作确实出现：\n"
        f"{actions_text}\n\n"
        f"每个动作的详细定义（必须逐条对照）：\n\n"
        f"## 追球\n"
        f"- 核心定义：狗有明确意图和行动去接近、追逐或拦截一个未被叼住的球。\n"
        f"- 属性（关键视觉线索）：\n"
        f"  1. 球存在（步骤1不是'完全没有看到球'）\n"
        f"  2. 球不在狗嘴里\n"
        f"  3. 狗的身体朝向球的方向运动（可以是跑、快走、小跑、冲刺）\n"
        f"  4. 狗与球之间的距离在缩短，或者有明显的追逐动作（如转向、加速、扑向球）\n"
        f"- 判定宽松原则：\n"
        f"  - 不要求狗必须'快速奔跑'，小跑、快走、甚至明显加速走向球都算追球。\n"
        f"  - 不要求'兴奋状态'，只要动作明确朝向球即可。\n"
        f"  - 狗从远处冲向球，即使中途被叫停或球被拿走，只要产生了追逐动作，就算追球。\n"
        f"- 不算追球的情况：\n"
        f"  - 球已经在狗嘴里\n"
        f"  - 狗只是看着球但没有移动\n"
        f"  - 狗缓慢、漫无目的地走动，不是朝向球\n"
        f"  - 狗在玩其他东西，球只是背景\n\n"
        f"## 拉球\n"
        f"- 定义：狗咬住球后，身体向后或向侧面发力拉扯，有明显的对抗性动作。\n"
        f"- 必要条件：\n"
        f"  1. 球在狗嘴里\n"
        f"  2. 狗身体后撤、头部后拉、四肢用力踩地\n"
        f"- 注意：只咬住球但不拉扯，不算拉球（可能属于犬咬）。\n\n"
        f"## 犬咬\n"
        f"- 定义：狗嘴里含着或叼着球，但没有持续的拉扯发力。\n"
        f"- 必要条件：\n"
        f"  1. 球在狗嘴里（必须看到球物体或轮廓在口部）\n"
        f"  2. 狗闭嘴含住或轻微咬合，但无向后拉扯动作\n"
        f"- 绝对不算犬咬的情况：\n"
        f"  - 视频中没有球\n"
        f"  - 狗只是张嘴或空咬，嘴里没有物体\n\n"
        f"全局判定铁律：\n"
        f"1. 步骤1的球位置结论是最高优先级。如果步骤1写'完全没有看到球'，则追球、拉球、犬咬全部判'未出现'。\n"
        f"2. 追球、拉球、犬咬三者互斥：\n"
        f"   - 如果球在嘴里 -> 可能为拉球或犬咬，绝不可能是追球。\n"
        f"   - 如果球不在嘴里 -> 可能为追球，绝不可能是拉球或犬咬。\n"
        f"3. 当狗明显冲向球（即使速度不快），必须判为追球。\n"
        f"4. 如果狗的行为不属于上述任何动作，请写'不属于上述动作'并描述实际行为。\n\n"
        f"判定格式（必须先写步骤1球位置结论）：\n"
        f"- 步骤1球位置结论：[在嘴里/在地上/在空中/被拿着/没有看到球]\n"
        f"- 追球：出现/未出现。理由：……（请说明球是否在嘴里、狗是否朝球移动、是否有距离缩短）\n"
        f"- 拉球：出现/未出现。理由：……\n"
        f"- 犬咬：出现/未出现。理由：……\n\n"
        f"[步骤3] 时间区间（仅当动作出现时）\n\n"
        f"[重要] 视频画面上可能有时间水印（例如 14:38:45 或 15:03:38），必须完全忽略！\n"
        f"禁止输出任何包含冒号(:)的时间格式。只允许输出数字（秒），可以有小数。\n\n"
        f"正确的计时方式：\n"
        f"- 从本视频片段的第一帧算起，时间为 0 秒。\n"
        f"- 观察动作实际发生的相对时刻。例如：\n"
        f"   - 片段总长 10 秒。狗从第 2.0 秒开始冲向球，第 3.5 秒咬住球 -> 输出 start=2.0, end=3.5\n"
        f"   - 狗从第 0 秒就已经叼着球，直到第 5 秒松开 -> 输出 start=0.0, end=5.0\n"
        f"   - 整段都没有球 -> 所有 start=null, end=null\n\n"
        f"错误示例（绝对禁止）：\n"
        f"- \"14:38:45 到 14:38:47\"\n"
        f"- start: 143845, end: 143847\n"
        f"- \"从第14分38秒开始\"\n\n"
        f"正确示例：\n"
        f"- 追球：2.0 秒到 3.5 秒\n"
        f"- 拉球：null 到 null (未出现)\n"
        f"- 犬咬：0.0 秒到 5.0 秒\n\n"
        f"先写文字说明每个动作的起止时间（必须用秒数，禁止使用时间戳）：\n"
        f"- 追球：未出现\n"
        f"- 拉球：3.2 秒到 4.5 秒\n"
        f"- 犬咬：未出现\n\n"
