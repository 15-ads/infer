...
def build_prompt(actions, duration, use_detailed=False):
    ...
    prompt = (
        ...
        f"请明确写出一句话回答：\n"
        f"- 球在哪里？\n"
        f"  - 在狗的嘴里（已叼住）——必须看到球物体在狗口中\n"
        f"  - 在地上（狗未接触）——必须看到球物体在地面\n"
        f"  - 在空中/被抛出（狗正在追逐）——必须看到球在空中\n"
        f"  - 被训导员拿在手里——必须看到球被人拿着\n"
        f"  - 完全没有看到球——视频中任何一帧都没有出现球物体\n\n"
        f"**极度重要：如果你没有在视频中看到任何球状的物体，必须写‘完全没有看到球’。**\n"
        f"**注意：狗做出张嘴、咬合、咀嚼等动作，但嘴里没有球物体，这不属于‘球在嘴里’。**\n\n"
        f"然后，用一两句话描述狗正在做什么（例如：奔跑、慢走、站立、嗅闻、跳跃、空咬、张嘴等）。\n\n"
        f"【步骤2】判定动作是否出现（必须写文字 + 理由）\n\n"
        f"你需要从下面列表中判断哪些动作**确实出现**：\n"
        f"{actions_text}\n\n"
        f"**全局硬性规则（违反任意一条，对应动作直接判‘未出现’）：**\n"
        f"- 如果步骤1中球的位置是‘完全没有看到球’，则追球、拉球、犬咬**全部**直接判‘未出现’，无需检查任何其他条件。\n"
        f"- 犬咬的定义核心：必须**实际看到球物体在狗嘴里**。狗只是张嘴、空咬、咀嚼动作但嘴里没有球 → 绝对不算犬咬。\n"
        f"- 追球的前提：球存在（不是‘没有看到球’）且球不在嘴里，同时狗在快速奔跑追逐。\n"
        f"- 拉球的前提：球在嘴里 + 狗身体向后发力拉扯。\n\n"
        f"每个动作的详细定义：\n\n"
        f"## 追球\n"
        f"- **必要条件（全部必须满足）**：\n"
        f"  1. 球存在（视频中能看到球）\n"
        f"  2. 球不在狗嘴里\n"
        f"  3. 狗在奔跑或快速移动，朝向球的方向\n"
        f"  4. 狗表现出兴奋状态\n"
        f"- **不算追球的情况**：慢悠悠走向球、只是看着球、试图接近但未奔跑、球在嘴里、没有球。\n\n"
        f"## 拉球\n"
        f"- **必要条件（全部必须满足）**：\n"
        f"  1. 球在狗嘴里\n"
        f"  2. 狗身体向后倾斜或头部向后拉扯\n"
        f"  3. 有明显的发力拉扯动作（四肢抓地、颈部后撤）\n"
        f"- **不算拉球的情况**：只是嘴里有球但没有拉扯动作。\n\n"
        f"## 犬咬\n"
        f"- **必要条件（全部必须满足）**：\n"
        f"  1. **球物体在狗嘴里**（必须能看到球，不能只是狗嘴闭合或张合）\n"
        f"  2. 狗含住或咬住球\n"
        f"  3. 没有连续的向后拉扯动作（如果有拉扯，则属于拉球）\n"
        f"- **绝对不算犬咬的情况**：\n"
        f"  - 视频中没有出现任何球\n"
        f"  - 狗只做出张嘴动作但嘴里没有球\n"
        f"  - 狗在空咬空气\n"
        f"  - 狗嘴里有其他物品（不是球）\n\n"
        f"**判定铁律：**\n"
        f"1. 步骤1的球位置结论是最高优先级判定依据。\n"
        f"2. 如果步骤1写的是‘完全没有看到球’，你必须在步骤2中输出：追球=未出现，拉球=未出现，犬咬=未出现。\n"
        f"3. 即使狗做出了类似‘咬’的动作，但没有球物体，也不能判任何与球相关的动作。\n"
        f"4. 如果狗的行为不属于上述动作，请写‘不属于上述动作’并描述实际行为。\n\n"
        f"**请按以下格式写出判定文字（必须首先写出步骤1的球位置结论）：**\n"
